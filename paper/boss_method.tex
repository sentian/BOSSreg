 %!TEX root = boss.tex
\section{BEST ORTHOGONALIZED SUBSET SELECTION}
\label{sec:boss}
With a general $X$, BS is not computationally feasible for large problems. In this section, we propose a LS-based subset selection method, the best orthogonalized subset selection (BOSS), that has computational cost of the same order as a multiple regression on all of the predictors. 
